...
const ip = "127.0.0.1";
const port = "1500";
const api = "http://localhost:1501/api/bpi?start=2018-01-01&end=2018-07-31";
...
if (!window.WebSocket) {
  document.write("<p>Sorry, your browser doesn't support websocket.</p>");
}
...
setInterval(() => {
  if (connection.readyState !== 1) {
    document.write(
      "<p>ERROR!!! Unable to communicate with the WebSocket server.</p>"
    );
    document.location.reload();
  }
}, 3000);

// most important part - incoming ws messages
connection.addEventListener("message", msg => {
  // try to parse JSON message. Because we know that the server
  // always returns JSON this should work without any problem but
  // we should make sure that the massage is not chunked or otherwise damaged.
  let msgData = null;
  try {
